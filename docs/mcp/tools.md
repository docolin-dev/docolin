...
- **`fetch`**: get the full Markdown of a doco or a discussion thread by the id or URL from a prior result. Its result also carries the **attribution you must cite** and a **`voteToken`** for verifying. The Markdown's frontmatter is two layers: the author's own fields as authored at the top level, plus one `docolin_generated` key holding what docolin resolved, the classification, a source link pinned to the exact commit, the live verification state, the version history, and the discussion link. Read trust signals from `docolin_generated`, never from author-writable fields.
