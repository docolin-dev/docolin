<script lang="ts">
  ...
  import type { DocoViewData } from "$lib/doco/viewer-data";
  ...
  let {
    ...
    data: DocoViewData;
    // The canonicalized-markdown endpoint for this doco. Route-specific (the
    // live route's /raw/... path, the preview route's local source), so it is
    // passed in. Shared by the copy-markdown button and the "View raw markdown"
    // menu item so they can never disagree.
    rawHref: string;
    // Local-folder preview: the doco isn't published, so the online fetches are
    // skipped and server-record actions (stamp, report, discussions, edit and
    // view-source on the forge) are disabled or hidden.
    preview?: boolean;
  } = $props();
  ...
  $effect(() => {
    ...
    if (data.playground || preview) return;
  ...
  $effect(() => {
    ...
    void doco.versionId;
    if (data.playground || preview || tags.length === 0) return;
    return trackDwell({
      onThreshold: () => {
        recordSetup(tags);
      },
  ...
  const editHref = $derived(
    !preview && data.pathInSource !== null && data.gitSource.repoUrl !== ""
...
<div class="flex gap-10 px-6 pt-24 pb-10">
  ...
  <article class="min-w-0 flex-1">
    <div class="mx-auto max-w-3xl">
      ...
      <header class="mb-10">
        ...
        <div class="flex items-start justify-between gap-4">
          ...
          <div class="mt-1 flex shrink-0 items-center gap-2">
            ...
            <DropdownMenu.Root>
              ...
              >
                ...
                {#if signedIn}
                  ...
                  <DropdownMenu.Item onSelect={openReport} disabled={preview}>
                    <Flag class="size-4" />
                    {m.doco_report()}
                  </DropdownMenu.Item>
                  {#if canModerate}
            ...
            {#if preview}
              <span title={m.preview_action_disabled()}>
                <button
                  type="button"
                  disabled
      ...
      {#if !data.playground}
        <StampPrompt versionId={doco.versionId} score={liveScore} {signedIn} {preview} />
      {/if}
      ...
      >
        ...
        {#if preview}
          <span title={m.preview_action_disabled()} class="shrink-0">
            <button
              type="button"
              disabled
              class="bg-primary/60 text-primary-foreground inline-flex w-full cursor-not-allowed items-center justify-center gap-1.5 px-4 py-2 text-sm font-medium opacity-70"
            >
